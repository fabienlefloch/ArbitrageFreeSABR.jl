...
function impliedVolatilityLiRationalGuess(price::T, forward::T, strike::T, timeToExpiry::T, isCall::Bool, df::T) where {T}
	c = price / forward / df
	ex = forward / strike
	x0 = log(ex)
	x = x0
...
function impliedVolatilityLiSORTS(price::T, forward::T, strike::T, timeToExpiry::T, isCall::Bool, df::T,
	impliedVolGuess::T, tolerance::T, maxIterations::Int) where {T}
	c = price / forward / df
	ex = forward / strike
	x0 = log(ex)
	x = x0
